...
* **High-Speed UDP Communication** Since `cloudflared+wstunnel` and `ssh -R`/`ssh -L` use TCP, they check for data corruption during transmission. However, the underlying protocols should handle data integrity checks without the need for the tunnel itself to implement them. Furthermore, if the tunnel’s error checking overlaps with the internal error checking, the network becomes flooded with verification packets, preventing the actual data from being transmitted and slowing down communication (TCP meltdown). Therefore, after exploring whether there was a way to use UDP for the tunnel, I concluded that a VPN was the optimal solution.
...
**Subsequent connections:** After the first run, a `ttstate.json` file is generated. You no longer need your API key!
```bash
...
### コンセプトと強み
* **同床異夢なVPN:** 従来のVPNは「参加者＝信頼できる仲間」が前提ですが、このツールではVPN内部をゼロトラストに扱います。外部のTailscaleサーバーを借りて調整を行う以上、中間者攻撃のリスクもゼロではないからです。DNSによる名前解決すらなく、明示的に設定しない限り、ネットワーク内からあなたのPCに触れることはできません。「誰かをVPNに招待したら、PC全体を見られてしまうのでは？」という心配は無用です。
...
**2回目以降の起動:** 一度実行すると `ttstate.json` という状態ファイルが生成されるため、以降はAPIキー（`-K`）の入力が不要になります。
```bash
./tunnneji-tail -S 443:localhost
```

> **メモ:** `ttstate.json` 以外の場所に状態ファイルを保存・読み込みしたい場合は、`-T <ファイルパス>` オプションを使って指定します。

### 2. 共有されたサーバーを見る（アクセス側: Bさん）
Bさんが、Aさんの共有したサーバー（ポート `443`）を見に行きたい場合。Bさんは、AさんのVPN IPとポートを、自分のローカルポート（ここでは `10443`）にマッピングします。
```bash
./tunnneji-tail -C 10443:<AさんのIPやドメイン>:443 -K <apiキー>
```
